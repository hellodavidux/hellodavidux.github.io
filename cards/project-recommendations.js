...
(function () {
  ...
  function renderCard(project, index) {
    ...
    return `
      <div class="w-full hoverable ${cardClass} flex-1 p-8 bg-white rounded-lg shadow-[0px_0px_2px_0px_rgba(0,0,0,0.12)] shadow-[0px_2px_4px_0px_rgba(0,0,0,0.14)] flex justify-start items-center gap-8 transition-all duration-300 hover:shadow-lg">
        <a href="${project.file}" class="flex justify-start items-center gap-8 w-full">
          <div class="w-40 h-28 rounded-lg shadow-[0px_0px_1.5px_0px_rgba(255,255,255,0.12)] shadow-[0px_6px_12px_0px_rgba(0,0,0,0.14)] inline-flex flex-col justify-end items-center overflow-hidden">
            <div class="w-full h-full overflow-hidden">
              ${renderThumbnail(project)}
            </div>
          </div>
          <div class="flex-1 justify-center text-zinc-950 text-2xl font-black ">${project.title}</div>
        </a>
      </div>
    `;
  }
